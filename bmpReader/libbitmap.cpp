...
public: // validation
	...
	{
		return (m_mark == markBM && m_length > 26 && m_offset >= 26 && m_length > m_offset && m_reserved[0] == 0 && m_reserved[1] == 0);
	}
		...
		else if (depth == bpp16)
		{
			for (size_t y = 0; y < height; ++y)
			{
				for (size_t x = 0; x < width; ++x)
				{
					...
					start[y * internal_stride + x*internal_depth / 8] = t;
					t = tmp & 0x7E0;
					t = ((double)t / 0x3F) * 0xFF;
					start[y * internal_stride + x*internal_depth / 8 + 1] = t;
					t = tmp& 0x1F;
					t = ((double)t / 0x1F) * 0xFF;
					start[y * internal_stride + x*internal_depth / 8 + 2] = t;
					start[y * internal_stride + x*internal_depth / 8 + 3] = 0;
				}
				if (padding && !read_object(file, dummy, padding))
					return Err::mode;
...
Err Bitmap::save(const char * file_name) const
{
	...
	const size_t buffer_size = ::get_pixel_array_size(m_width, m_height, bpp32);
	const ptrdiff_t buffer_stride = topdown ?
		::get_stride(m_width, bpp32) : -ptrdiff_t(::get_stride(m_width, bpp32));
	uint8_t * const buffer_start = topdown ? m_buffer : m_buffer + buffer_size + buffer_stride;

	const std::streamsize padding = std::streamsize(::get_padding(m_width, bpp32));
	const std::streamsize line_size = std::streamsize(::get_line_size(m_width, bpp32));
